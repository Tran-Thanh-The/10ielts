...
export class FilesGoogleDriveService {
  ...
  constructor(
    ...
    try {
      const clientId = this.configService.get("file.googleDriveClientId", {
        infer: true,
      });
      const clientSecret = this.configService.get(
        "file.googleDriveClientSecret",
        { infer: true },
      );
      const redirectUri = this.configService.get(
        "file.googleDriveRedirectUri",
        { infer: true },
      );
      const refreshToken = this.configService.get(
        "file.googleDriveRefreshToken",
        { infer: true },
      );
  ...
  private async getFileUrl(fileId: string, mimeType: string): Promise<string> {
    try {
      ...
      if (mimeType.startsWith("video/") || mimeType.startsWith("audio/")) {
        const apiKey = await this.configService.get("file.googleDriveApiKey", {
          infer: true,
        });
  ...
  async create(file: Express.Multer.File): Promise<{ file: FileType }> {
    ...
    try {
      ...
      const directLink = await this.getFileUrl(
        driveFile.data.id,
        file.mimetype,
      );
  ...
  async delete(file: FileType): Promise<void> {
    ...
    try {
      ...
      console.error(
        "Error during file delete process:",
